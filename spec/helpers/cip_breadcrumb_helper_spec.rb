...
describe CipBreadcrumbHelper, type: :helper do
  ...
  let(:programme) { FactoryBot.create(:core_induction_programme, course_year_one: course_year) }
  ...
  describe "#course_lesson_breadcrumbs" do
    context "creating a course_lesson and module is not yet known" do
      ...
      it "returns Home => /dashboard, Create lesson" do
        expect(helper.course_lesson_breadcrumbs(user, course_lesson)).to eql([["Home", "/dashboard"], ["Create lesson"]])
      end
    end
  end
end
